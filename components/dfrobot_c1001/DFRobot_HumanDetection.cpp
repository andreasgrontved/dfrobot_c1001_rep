 ...
 * @brief Implementation of DFRobot_HumanDetection adapted for ESP-IDF.
...
// ESP-IDF logging tag
static const char *TAG = "HumanDetection";

// Replace Arduino's delay() with FreeRTOS vTaskDelay
#define delay(ms) vTaskDelay(pdMS_TO_TICKS(ms))

// Provide a millis() function using FreeRTOS tick count
uint32_t millis() {
...
void DFRobot_HumanDetection::dmInstallAngle(int16_t x, int16_t y, int16_t z)
{
    ...
    sendBuf[3] = y & 0xff;
    sendBuf[4] = (z >> 8) & 0xff;
    sendBuf[5] = z & 0xff; // Note: fixed potential copy-paste error (was y again)
    if (getData(0x06, 0x01, 6, sendBuf, readBuf) == 0)
    {
        // Command sent; no further action
    }
}
